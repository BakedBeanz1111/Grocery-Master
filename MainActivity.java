...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, View.OnLongClickListener {
    ...
    private static ArrayList<GroceryList> deletedList = new ArrayList<>(); //holds deleted grocery lists
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle("Groceries (" + mainList.size() + ")");
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.retrieveDeleted:
                AlertDialog.Builder builder = new AlertDialog.Builder(this);
                builder.setTitle("Choose grocery list(s) to retrieve");
                if (!deletedList.isEmpty()) {
                    String[] lst = new String[deletedList.size()];
                    for (int i = 0; i < lst.length; i++) {
                        lst[i] = deletedList.get(i).getTitle();
                    }
                    String[] lst2 = new ArrayList<String>() {
                        {
                            add("add all");
                            addAll(Arrays.asList(lst));
                        }
                    }.toArray(new String[0]);
                    builder.setItems(lst2, (dialog, which) -> {
                        if (which == 0) {
                            ...
                            mainList.add(deletedList.get(which-1));
                            deletedList.remove(which-1);
                            Collections.sort(mainList, comp);
                            listAdapter.notifyDataSetChanged();
                        }
                    });
            ...
            case R.id.delete:
                if (!mainList.isEmpty()) {
                    ...
                    builder2.setTitle("Are you sure you want to delete all Lists?");
                    builder2.setPositiveButton("YES", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            deletedList.addAll(mainList);
                            mainList.clear();
                            listAdapter.notifyDataSetChanged();
                            setTitle("Groceries (" + mainList.size() + ")");
                        }
    ...
    public boolean onLongClick(View v) {
        ...
        selection = mainList.get(position);
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Delete List " + (selection.getTitle()) + "?");
        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                if (!deletedList.contains(selection)) {
